...
const shutdown = async (reason: string) => {
  ...
  shuttingDown = true;
  console.log(`Shutting down gateway (${reason})...`);

  runtime.autoRegisterReplenisher.stop();
  runtime.poller.stop();
  runtime.codexConfigAutoManager.restoreForShutdown();

  await runtime.app.close().catch(() => undefined);
  await runtime.mockUpstream.app.close().catch(() => undefined);
};
...
const boot = async () => {
  try {
    ...
    console.log("  GET  /admin/codex-auto-config");
    console.log("  PUT  /admin/codex-auto-config");
    ...
    console.log("  POST /admin/chatgpt-register/start");
    console.log("  GET  /admin/chatgpt-register/:taskId");
    console.log("  POST /admin/chatgpt-register/:taskId/cancel");
    ...
    console.error("Gateway boot failed:", error);
    await shutdown("boot-failed");
    process.exit(1);
  }
};
